...
Laurent Oudre pour son aiguillage avisé,
...
% MANQUE DES LABELS
	...
	on considère les signaux comme la réalisation d'une suite (finie) de variables aléatoires (possiblement multivariées).
%Proposer ici des références bibliographiques...
 ...
 \begin{figure}[h]
 	\centering
 	\includegraphics{diagramme_cusum_dikt}
 	\caption{Schéma reprenant l'algorithme de segmentation dichotomique : en vert les ruptures choisies,  en rouge les éligibles ; $t_{2,2}$ vient d'être choisie}
 \end{figure}

	L'implémentation dichotomique du CUSUM consiste alors à calculer un arbre de la façon suivante :
	\\
	\begin{itemize}
	\item Maintenir un ensemble de ruptures choisies initialement vide (les nœuds intérieurs).
	...
	\item En extraire la rupture $t_0$ qui a le plus grand ratio de vraisemblance.
	\item Calculer les deux ruptures -- qui sont alors éligibles -- en appliquant le CUSUM entre la rupture choisie précédant chronologiquement $t_0$ et $t_0$ et entre $t_0$ et la rupture choisie suivant chronologiquement $t_0$.
	...
	Sauf que l'arbre complet tombe dans un grave écueil : plusieurs de ses sous-arbres sera vite confiné à calculer des ruptures là où elles sont minimes.
	...
	\\
		...
		Observons en détails la manière de procéder sur un exemple (signal \ref{test_multi_dikt})
...
%		\vspace{2cm}
	...
	Les valeurs de l'espace entre ruptures sont tirés uniformément sur l'intervalle ouvert de $\mathbb R^{+*}$ qui s'étend de l'espace minimal au double de l'espace moyen moins l'espace minimal.
	Les paramètres de chaque distribution normale sont tout deux -- moyenne et écart-type -- tiré uniformément sur $]0\,;1[$.
...
%		\vspace{2cm}
	...
	\label{score_min}
	\end{figure}	
	L'espace moyen est réglé à 500 points (3 à 4000 points par acquisitions présentant environ 6 ruptures).	
	...
	\begin{figure}[hp]
		...
		\begin{subfigure}[t]{.49\textwidth}
			...
			\label{}
		\end{subfigure}
	\caption{6 ruptures sur un premier signal physiologique : en bleu l'accélération enregistrée à la ceinture ; en vert l'accélération enregistrée au pied droit ; en rouge la rotation propre enregistrée à la ceinture}
	\end{figure}
	...
	\begin{figure}[hp]
		...
		\begin{subfigure}[t]{.49\textwidth}
			...
			\label{}
		\end{subfigure}
	\caption{6 ruptures sur un deuxième signal physiologique : en bleu l'accélération enregistrée à la ceinture ; en vert l'accélération enregistrée au pied droit ; en rouge la rotation propre enregistrée à la ceinture}
	\end{figure}
